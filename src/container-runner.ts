...
async function buildContainerArgs(
  ...
  // Memory limit — per-group override falls back to CONTAINER_MEMORY_MB.
  const memoryMb = containerConfig.memoryMb ?? CONTAINER_MEMORY_MB;
  ...
  // Native credential proxy — point the container's Anthropic SDK at the
  // proxy so credentials never leave the host. The proxy injects auth
  // headers based on .env (.env is never mounted into the container).
  args.push('-e', `ANTHROPIC_BASE_URL=http://${CONTAINER_HOST_GATEWAY}:${CREDENTIAL_PROXY_PORT}`);
  if (detectAuthMode() === 'oauth') {
    // Placeholder: container's CLI carries this as its OAuth token; the
    // proxy swaps it for the real token on the exchange request.
    args.push('-e', 'CLAUDE_CODE_OAUTH_TOKEN=placeholder');
  } else {
    // API-key mode: container needs *some* key set or the SDK refuses to start.
    // The proxy will overwrite x-api-key on every request anyway.
    args.push('-e', 'ANTHROPIC_API_KEY=placeholder');
  }
  log.debug('Credential proxy injected', { containerName });
